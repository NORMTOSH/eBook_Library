...
from store.models import Book  # Import your Book model or relevant model
...
def pdf_view(request, book_id):
    try:
        book = Book.objects.get(id=book_id)  # Replace with how you retrieve the book
    ...
    return render(request, 'store/pdf_view.html', context)

def recent_books(request):
    # Retrieve the 5 most recent books by publication date
    newbooks = Book.objects.order_by('-publication_date')[:5]
    return render(request, 'store/index.html', {'newbooks': newbooks})
